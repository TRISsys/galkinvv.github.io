...
Работает, но в редких случаях не инициализируется. на плате множество снятых конденсаторов, но работает. После лежания и кардинальной чистки - артефакты памяти. Со стороны GPU отсуствующих элементов не обнаружено, а с обратной:
 * 1 сдвинутый конденсатор на PCIe
 * 1 сколотый резистор между GPU и наклейкой  A100520002. На непохожем reference 470 он 10КОм, а на более похожем GTX570 - 321Ом. Без резистора - 0.3 - 2.0 МОм в зависимости от соседних измерений (видимо открываются транзисторы). Одна нога на земле, другая идёт в gpu.
 * 4 больших конденсатора в районе доп. питания.
 * 2 средних конденсатора по верхней линии памяти
 * 4 мелких конденсаторов в районе оборотной стороны M7, M8, из которых 2 далёких друг от друга НЕ по питанию памяти (до земли 114КОм, 368Ом).
