...
В текстовом режиме некоторые символы мусорные. При загрузке драйвера сыпет ошибки и падает драйвер. Программный анализ показал, что взводится 2-й бит (пробелы превращаются в $) для многих байтов. Поочерёдный RESET модулей показал, что по всей видимости пролема в микросхеме памяти M2.
